...
//       } else {
//         result = await signupUser({ email, password, username: username,userType:2 }).unwrap();
//       }
...
//           <button 
//             className={`px-6 py-2 text-lg font-semibold rounded-md transition-all ${isLogin ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-800"}`} 
...
//           )}
//           <button 
//             className="bg-blue-600 text-white py-3 rounded-md font-bold hover:bg-blue-700 transition-all" 
//             disabled={loading}
//           >
...
//         {error && <p className="text-red-500 text-sm mb-2">{error}</p>}
//         {success && <p className="text-green-500 text-sm mb-2 bg-green-100 p-2 rounded-md">{isLogin ? "Login Successful!" : "Registration Successful!"}</p>}
